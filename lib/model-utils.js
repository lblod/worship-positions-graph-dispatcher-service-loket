...
export function createQueryForPath(path, subject, vendor) {
  ...
  wherePart.push(`VALUES ${subjectChain[0]} { ${rst.termToString(subject)} }`);
  ...
  for (let i = 0; i < path.length; i++) {
    filterPart.push(
      `FILTER EXISTS { ${subjectChain[i]} rdf:type ${rst.termToString(path[i].type)} . }`,
      //`FILTER EXISTS { ${subjectChain[i]} <http://www.w3.org/ns/prov#wasGeneratedBy> <http://lblod.data.gift/id/app/lblod-harvesting> . }`,
      `FILTER EXISTS { ${subjectChain[i]} <http://www.w3.org/ns/prov#wasAssociatedWith> ${rst.termToString(vendor)} . }`,
    );
  }
