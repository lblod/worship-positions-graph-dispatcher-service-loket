...
export async function getSubModelData(subject, type, allowedOrganisationUUIDs) {
  const store = new N3.Store();
  const queries = createQueriesForSubmodel(
    subject,
    type,
    allowedOrganisationUUIDs,
  );
  for (const query of queries) {
    const partialStore = sts.getDataFromConstructQuery(query);
    store.addQuads(partialStore);
  }
  return store;
}

export function createQueriesForSubmodel(
  subject,
  type,
  allowedOrganisationUUIDs,
) {
  ...
  for (const sm of submodels) {
    ...
    const query = createQueryForPath(path, subject, allowedOrganisationUUIDs);
    queries.push(query);
  }
  return queries;
}

export function createQueryForPath(path, subject, allowedOrganisationUUIDs) {
  ...
  const allowedOrganisationUUIDsForValues = allowedOrganisationUUIDs
    .map((uuid) => {
      return `"${uuid}"`;
    })
    .join('\n');
  ...
  const query = `
    ...
    WHERE {
      ...
      VALUES ?orgUuid {
        ${allowedOrganisationUUIDsForValues}
      }
      FILTER (REGEX(STR(?g), STR(?orgUuid)))
    }
  `;
