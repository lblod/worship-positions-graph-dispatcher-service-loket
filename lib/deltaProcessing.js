...
async function dispatch(subjectsWithTypes) {
  ...
  for (const individual of subjectsWithTypes) {
    ...
    //TODO: This works only per type, not per type and path. First check if this is needed from business.
    console.log(`Trying to dispatch info about ${subject.value} for type: ${type.value}`);
    const config = pta.find(cfg => cfg.type.value === type.value);

    if (config && !config.allowedInMultipleOrgs && organisationUUIDs.length > 1) {
    ...
    } else if (config && config.allowedInMultipleOrgs && organisationUUIDs.length > 0) {
      ...
      results.push({
        ...
        reason:
          `No organisation found.
           This could be normal. This subject is tried again later.
           It was about subject: ${subject.value} for type: ${type.value}
          `,
      });
    }
  }
