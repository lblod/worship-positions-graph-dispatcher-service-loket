...
import * as mut from './model-utils';
...
async function dispatch(subjectsWithTypes) {
  ...
  for (const { subject, type } of subjectsWithTypes) {
    ...
    const vendor = await getVendorFromSubject(subject);

    if (!vendor) {
      //No vendor? Then we can't dispatch the data.
      const insertGraph = namedNode(env.TEMP_GRAPH_INSERTS);
      const discardGraph = namedNode(env.TEMP_GRAPH_DISCARDS);
      await moveSubjectBetweenGraphs(subject, insertGraph, [discardGraph]);
      results.push({
        success: false,
        mode: 'Insert',
        subject,
        type,
        reason:
          'There is no vendor for this information. This data cannot be dispatched.',
      });
      continue;
    }

    const allowedOrganisationUUIDs =
      await getAllowedOrganisationUUIDsForVendor(vendor);
    //TODO: This works only per type, not per type and path. First check if
    //this is needed from business.
    ...
    if (
      ...
      for (const organisationUUID of organisationUUIDs) {
        //We need to sort out in wich graph the data needs to be put in.
        //Inserting in both graphs is a temporary measure to make things easier
        //for now.

        //Is vendorURI a valid vendor for this organisation?
        if (allowedOrganisationUUIDs.includes(organisationUUID)) {
          const organisationGraphs = [
            `${env.ORGANISATION_GRAPH_PREFIX}${organisationUUID}/LoketLB-eredienstMandaatGebruiker`,
            `${env.ORGANISATION_GRAPH_PREFIX}${organisationUUID}/LoketLB-eredienstBedienaarGebruiker`,
          ].map(namedNode);
          const insertGraph = namedNode(env.TEMP_GRAPH_INSERTS);
          //Execute move query for all data of that `subject` to graph constructed
          //from the UUID
          await moveSubjectAndRelatedBetweenGraphs(
            subject,
            type,
            insertGraph,
            organisationGraphs,
          ...
          //Vendor is not allowed to publish data for this organisation
          //Instead of ignoring the data and resulting in this error message,
          //we should place the data in a discard graph so that it can still be
          //used to find paths, but will not be published in the organisation.
          //This data will be ignored in next iterations of this service.
          const insertGraph = namedNode(env.TEMP_GRAPH_INSERTS);
          const discardGraph = namedNode(env.TEMP_GRAPH_DISCARDS);
          await moveSubjectBetweenGraphs(subject, insertGraph, [discardGraph]);
          results.push({
            ...
            type,
            reason:
              'This organisation has been blocked. This means that this vendor is not allowed to publish data for this organisation.',
            organisationUUID,
            vendor,
          });
        }
...
/**
 * Get the attached vendor for a specific subject from the triplestore. All
 * harvested subjects should be tagged with this kind of provenance.
 *
 * @async
 * @function
 * @param {NamedNode} subject - A given subject to get the vendor from.
 * @returns {NamedNode} Vendor that published this subject.
 */
async function getVendorFromSubject(subject) {
  const response = await mas.querySudo(`
    ${env.SPARQL_PREFIXES}
    SELECT ?vendor WHERE {
      BIND (${rst.termToString(subject)} AS ?subject) .
      ?subject prov:wasAssociatedWith ?vendor .
    }
    LIMIT 1`);
  const parser = new sjp.SparqlJsonParser();
  return parser.parseJsonResults(response).map((r) => r.vendor)[0];
}

const vendorOrgCache = new TTLCache({
  ttl: 3600 * 1000, //one hour
});

/**
 * For a vendor, get all the organisations they are allowed to publish data
 * for. This function is memoized with a cache that lives for an hour.
 *
 * @async
 * @function
 * @param {NamedNode} vendor - A vendor to get the organizational units that
 * this vendor is allowed to publish data for.
 * @returns {Array(NamedNode)} List of organisations.
 */
async function getAllowedOrganisationUUIDsForVendor(vendor) {
  const vendorString = rst.termToString(vendor);
  const maybeResult = vendorOrgCache.get(vendorString);
  if (maybeResult) {
    return maybeResult;
  } else {
    const response = await mas.querySudo(`
      ${env.SPARQL_PREFIXES}
      SELECT ?organisationUUID WHERE {
        BIND (${vendorString} AS ?vendor) .
        ?vendor muAccount:canActOnBehalfOf ?organisation .
        ?organisation mu:uuid ?organisationUUID .
      }
    `);
    const parser = new sjp.SparqlJsonParser();
    const parsedResults = parser
      .parseJsonResults(response)
      .map((r) => r.organisationUUID.value);
    vendorOrgCache.set(vendorString, parsedResults);
    return parsedResults;
  }
}
 ...
 * @param {NamedNode} originalGraph - Graph where data will be searched in and
 * removed.
 * @param {Iterable(NamedNode)} targetGraphs - Collection (Array, iterator, ...)
...
/**
 * @see moveSubjectBetweenGraphs
 *
 * Similar to moveSubjectBetweenGraphs, but first, all the data for the model
 * and related model elements is collected and copied at once. This uses a
 * user-defined hierarchical model of the data.
 *
 @async
 * @function
 * @param {NamedNode} subject - Subject for which child elements of the model
 * also need to be copied to the target graphs.
 * @param {NamedNode} type - Type of the subject.
 * @param {NamedNode} originalGraph - Original graph where the subjects exists.
 * More data is collected from other graphs, but all data for this subject and
 * related entities is removed from this graph afterwards.
 * @param {Iterable(NamedNode)} targetGraphs - Collection (Array, iterator,
 * ...) of graphs where the data should end up in.
 * @param {NamedNode} vendor - Vendor that publishes this data.
 * @returns {undefined} Nothing
 */
async function moveSubjectAndRelatedBetweenGraphs(
  subject,
  type,
  originalGraph,
  targetGraphs,
  ...
  //Get all the data for the subject and all the related submodel elements
  const data = await mut.getSubModelData(subject, type, vendor);

  //Insert collected data to organisation graphs
  for (const targetGraph of targetGraphs)
    await sts.insertData(data, targetGraph);

  //Delete data from source graph (with literal triple one by one due to
  //Virtuoso bug)
  const subjectData = await sts.getDataForSubject(subject, originalGraph);
  ////Remove triples without literals or untyped literals
  const literalTriples = [];
  subjectData.forEach((quad) => {
    if (quad.object.termType === 'Literal') literalTriples.push(quad);
  });
  subjectData.removeQuads(literalTriples);
  await sts.deleteData(subjectData, originalGraph);
  ////Remove triples with typed literals, one by one, due to a bug in Virtuoso
  for (const triple of literalTriples) {
    const deleteStore = new N3.Store();
    deleteStore.addQuad(triple);
