  ...
  for (const individual of subjectsWithTypes) {
    ...
    if (organisationUUIDs.length > 1) {
      ...
      const organisationGraphs = [
        namedNode(
          `${env.ORGANISATION_GRAPH_PREFIX}${organisationUUIDs[0]}/LoketLB-eredienstMandaatGebruiker`
        ),
        namedNode(
          `${env.ORGANISATION_GRAPH_PREFIX}${organisationUUIDs[0]}/LoketLB-eredienstBedienaarGebruiker`
        ),
      ];
