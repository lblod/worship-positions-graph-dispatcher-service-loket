...
async function dispatch(subjectsWithTypes) {
  ...
  for (const individual of subjectsWithTypes) {
    ...
    const { allowedInMultipleOrgs } = pta.find(cfg => cfg.type.value === type.value);

    if (!allowedInMultipleOrgs && organisationUUIDs.length > 1) {
    ...
    } else if (organisationUUIDs.length > 0) {
